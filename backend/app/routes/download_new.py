...
def build_poster_object_key(video_object_key: str) -> str:
    path = PurePosixPath(video_object_key)
    stem = path.stem
    parent = str(path.parent)
    return f"{parent}/{stem}.jpg"


def build_poster_url(bucket_name: str, object_key: str, request: Request) -> str:
    poster_key = build_poster_object_key(object_key)
    return build_obs_object_url(bucket_name, poster_key, request)
...
def get_recordings(
    ...
    for r in records:
        ...
        poster_url = build_poster_url(r.bucket_name, r.object_key, request)
